              ...
              <div class="modal-body">
                <form id="form-siswa" method="POST">
                  <div class="form-group">
                    ...
                    <input type="hidden" id="id_aspek" name="id_aspek" >
                    <input type="text" class="form-control" id="nama_aspek" name="nama_aspek" required>
                  </div>
                  <!-- <div class="form-group">
                    <label for="id_form">Form</label>             
                    <input type="text" class="form-control" id="id_form" name="id_form" required>
                  </div> -->
                  ...
                  <div class="form-group">
                    ...
                    <label for="folder">sub</label>
                    <input type="radio" id="bukan_folder" name="folder" value="0">
                    <label for="bukan_folder"> bukan sub</label><br>
                  </div>
  ...
  <script>
    ...
    function tampil_siswa(parent=0,nama="Home",back="false") {
      $('#tabel-siswa').DataTable({
        ...
        responsive: true,
        ajax: {
          url: "<?= base_url('admin/aspek/get_siswa'); ?>",
          type: "POST",
      ...
      document.getElementById('id_parent').value=parent; // biar inputan yang id nya = 'id_parent' bernilai=isi'parent'. isi dari 'parent' adalah id_aspek(liat di model).
      // berarti itu buat nampilin data yang setiap id_aspek dengan nilai parent_id nya sama. 
    ...
    function check_status() {
      switch (status) {
        case 'tambah':
          ...
          url = "<?= base_url('index.php/admin/aspek/save_siswa'); ?>";
          break;
        case 'edit':
          $('#btn-proses').text('Update Changes');
          url = "<?= base_url('index.php/admin/aspek/update_siswa'); ?>";
          break;
        case 'hapus':
          url = "<?= base_url('index.php/admin/aspek/delete_siswa'); ?>";
          break;
        case 'notif' :
        $('#btn-proses').text('Save Changes');
          url = "<?= base_url('index.php/admin/aspek/save_siswa'); ?>";
    ...
    function edit(id_aspek) {
      ...
      clear_form();
      $.ajax({
        url: "<?= base_url('index.php/admin/aspek/edit_siswa'); ?>",
        type: "GET",
        dataType: "JSON",
        data: {
          id_aspek: id_aspek
        ...
        success: function(x) {
          if (x.status == true) {
            ...
            $('#nama_aspek').val(x.data.nama_aspek);
    ...
    function proses() {
      check_status();
      parent=document.getElementById("id_parent").value; // ngeset biar inputan yang id nya = id_parent bernilai dengan isinya parent(id_aspek si parent.
      // posisi=parent;
      $.ajax({
        url: url,
        type: "POST",
